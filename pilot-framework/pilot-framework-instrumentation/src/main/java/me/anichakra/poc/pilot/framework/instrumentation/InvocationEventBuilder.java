 ...
 *
 */
public class InvocationEventBuilder {
	...
	private static final ThreadLocal<InvocationEventBuilder> CURRENT = new ThreadLocal<>();
	 ...
	 * @param id
	 */
	InvocationEventBuilder(String id) {
		this.id = id;
	}
	 ...
	 * @return
	 */
	public synchronized static InvocationEventBuilder getCurrent() {
		return CURRENT.get();
	}
	 ...
	 * @param current
	 */
	static synchronized void setCurrent(InvocationEventBuilder current) {
		if (getCurrent() == null)
			CURRENT.set(current);
	}
	...
	public void markIgnore(long durationToIgnore) {
		InvocationEventBuilder event = InvocationEventBuilder.getCurrent();
	...
	public Throwable getRootCause() {
		InvocationEventBuilder invocation = InvocationEventBuilder.getCurrent();
	...
	public InvocationEvent createInvocationEvent() {
		try {
			InvocationLineItem invocationLineItem = getCurrentLineItem();
			return new InvocationEvent(getId(), (InvocationLineItem)invocationLineItem.clone(), 
					getMetricMap());
		} catch (CloneNotSupportedException e) {
			return new InvocationEvent(getId(), new InvocationLineItem(), getMetricMap());
		}
	}
